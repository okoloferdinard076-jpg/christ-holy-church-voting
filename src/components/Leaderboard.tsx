...
export const Leaderboard: React.FC<LeaderboardProps> = ({
  ...
  return (
    <section className="py-16 bg-slate-50/70 border-b border-slate-200/80" id="leaderboard">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {!hasAnyVotes && (
          <div className="mb-8 p-4 rounded-2xl bg-white border border-blue-100 shadow-xs flex items-center gap-3 text-left">
              ...
              return (
                <div
                  ...
                  className={`rounded-2xl p-5 border flex flex-col justify-between relative overflow-hidden transition-all ${
                    rank === 1 && votes > 0
                      ? 'bg-gradient-to-b from-amber-500/10 via-white to-white border-amber-300 shadow-md ring-2 ring-amber-400/20'
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      {rank === 1 && votes > 0 ? (
                        ...
                        <span className="w-8 h-8 rounded-full bg-slate-100 text-slate-700 font-bold text-xs flex items-center justify-center">
                          #{rank}
                        </span>
                      )}
                    ...
                    <span className="text-xs font-bold text-blue-900 bg-blue-50 px-2.5 py-0.5 rounded-full">
                      {percentage}% of votes
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-3.5 my-2">
                    {candidate.image && candidate.image.trim() ? (
                      ...
                      <div className="w-14 h-14 rounded-full bg-blue-900/10 border-2 border-blue-950/20 flex items-center justify-center text-blue-950 font-black text-sm shadow-inner shrink-0">
                        {getInitials(candidate.name) || <User className="w-6 h-6" />}
                      </div>
                    )}
                    <div>
                      <h3 className="font-extrabold text-base text-blue-950 leading-tight">
                        {candidate.name}
                      </h3>
                      <p className="text-xs text-red-600 font-semibold">{candidate.state}</p>
                    </div>
                  </div>

                  <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
                    <div>
                      <div className="text-[11px] text-slate-500 font-medium">Approved Votes</div>
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-blue-900 hover:bg-blue-950 text-white text-xs font-bold transition-colors flex items-center gap-1 cursor-pointer"
                    >
                ...
                {sortedCandidates.map((candidate, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center gap-2.5 sm:gap-3">
                          {candidate.image && candidate.image.trim() ? (
                            ...
                            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-blue-900/10 border border-blue-950/15 flex items-center justify-center text-blue-950 font-bold text-xs shrink-0">
                          ...
                          <div className="min-w-0">
                            <button
                              ...
                              className="font-bold text-blue-950 hover:text-blue-700 transition-colors text-left text-xs sm:text-sm truncate block max-w-[130px] sm:max-w-none"
                            >
                              {candidate.name}
                            </button>
                      ...
                      <td className="py-3.5 px-4 font-semibold text-slate-700 hidden md:table-cell">
                        <span className="inline-block px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-800 text-xs font-medium">
                          {candidate.state}
                        </span>
                      </td>
                      <td className="py-3.5 px-4 text-right font-black text-blue-950 text-sm sm:text-base">
                        {votes.toLocaleString()}
                      </td>
                      ...
                      <td className="py-3.5 px-4 text-center">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-blue-900 hover:bg-blue-950 text-white font-bold text-xs tracking-wide transition-all shadow-2xs inline-flex items-center gap-1 cursor-pointer"
                        >
                          <Vote className="w-3.5 h-3.5 text-amber-400" />
                          <span>Vote</span>
                        </button>
